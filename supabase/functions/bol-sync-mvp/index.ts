...
async function token() {
  const r=await fetch("https://login.bol.com/token",{
    method:"POST",
    headers:{
      "Authorization":`Basic ${btoa(`${BOL_CLIENT_ID}:${BOL_CLIENT_SECRET}`)}`,
      "Accept":"application/json",
      "Content-Type":"application/x-www-form-urlencoded",
      "User-Agent":"MAT-Home-Dashboard/0.2"
    },
  ...
  try{
    ...
    for(const o of allOffers){
      ...
      const row={
        offer_id:o.offerId,
        ean:o.ean ?? null,
        title:o.reference || o.unknownProductTitle || o.ean || "Bol product",
        variant:o.reference ?? null,
        active:Boolean(nl?.forSale) && !o.onHoldByRetailer,
        current_price:price,
        stock:o.stock?.correctedStock ?? o.stock?.amount ?? null,
        fulfillment_method:o.fulfilment?.method ?? null,
        delivery_code:o.fulfilment?.schedule ?? null
      };
    ...
    for(let page=1;page<=4;page++){
      ...
      for(const summary of orders){
        ...
        const ord=(await upsert("orders",{
          bol_order_id:String(detail.orderId),
          ordered_at:detail.orderPlacedDateTime,
          status:"CURRENT",
        ...
        for(const item of items){
          ...
          if(offerId){
            const saved=await upsert("products",{
              ...
              title:item.product?.title || item.offer?.reference || item.product?.ean || "Bol product",
              variant:item.offer?.reference ?? null,
              active:prod?.active ?? true,
              current_price:Number(item.unitPrice ?? prod?.current_price ?? 0)||null,
              stock:prod?.stock ?? null,
              fulfillment_method:item.fulfilment?.method ?? prod?.fulfillment_method ?? null,
              delivery_code:prod?.delivery_code ?? null
            },"offer_id");
            prod=saved?.[0]||prod;
          ...
          await upsert("order_items",{
            ...
            status:item.quantityCancelled>0?"CANCELLED":(item.quantityShipped>0?"SHIPPED":"OPEN")
          },"bol_order_item_id");
          if(prod?.id && date){
    ...
    const activeOffers=allOffers.filter((o:any)=>{
      const nl=(o.countryAvailabilities||[]).find((x:any)=>x.countryCode==="NL");
      return Boolean(nl?.forSale) && !o.onHoldByRetailer;
    }).slice(0,40);
    ...
    for(const o of activeOffers){
      ...
      for(const name of ["PRODUCT_VISITS","BUY_BOX_PERCENTAGE"]){
        ...
        u.searchParams.set("offer-id",o.offerId);
        u.searchParams.set("period","DAY");
        u.searchParams.set("number-of-periods","14");
        u.searchParams.append("name",name);
        const insight=await bol(u.toString(),accessToken,10);
        for(const block of (insight?.offerInsights||[])){
          for(const p of (block?.periods||[])){
            const date=periodDate(p.period);
            if(!date) continue;
      ...
      for(const date of dates){
        ...
        await upsert("product_metrics",{
          product_id:prod.id,
          metric_date:date,
          visits,
          sales:sale.sales,
          conversion_rate:visits>0?Math.round((sale.sales/visits*100)*10000)/10000:0,
          revenue:Math.round(sale.revenue*100)/100,
          buy_box_percentage:buyBoxByDate.get(date)??null
      ...
      if(o.ean){
        ...
        for(const c of (comp?.offers||[])){
          ...
          const competitor=(await upsert("competitors",{
            ean:o.ean,
            seller_id:String(c.retailerId ?? ""),
            seller_name:c.retailerId?("Retailer "+String(c.retailerId)):null,
            offer_id:c.offerId,
            last_seen_at:new Date().toISOString()
          },"ean,seller_id,offer_id"))?.[0];
          if(competitor){
            await db("competitor_snapshots",{
              method:"POST",
              headers:{"Prefer":"return=minimal"},
              body:JSON.stringify({
                competitor_id:competitor.id,
                price:c.price ?? null,
                delivery_date:c.minDeliveryDate ?? null,
                is_best_offer:Boolean(c.bestOffer),
                availability:c.fulfilmentMethod ?? null
              })
            });
          }
        }
    ...
    for(const searchTerm of searchTerms){
      try{
        const u=new URL("https://api.bol.com/retailer/insights/search-terms");
        u.searchParams.set("search-term",searchTerm);
        u.searchParams.set("period","DAY");
        u.searchParams.set("number-of-periods","14");
        u.searchParams.set("related-search-terms","false");
        const insight=await bol(u.toString(),accessToken,10);
        const block=insight?.searchTerms;
        for(const p of (block?.periods||[])){
          const date=periodDate(p.period);
          if(!date) continue;
          const nl=(p.countries||[]).find((x:any)=>x.countryCode==="NL");
          const volume=Math.max(0,Math.round(Number(nl?.value ?? p.total ?? 0)));
          await upsert("search_term_metrics",{
            search_term:searchTerm,
            metric_date:date,
            search_volume:volume,
            country_code:"NL",
            updated_at:new Date().toISOString()
          },"search_term,metric_date,country_code");
          records++;
        }
      }catch(e){
        console.error("search term insight failed",searchTerm,String(e));
      }
    }
    ...
    await db("sync_runs",{
      method:"POST",
      headers:{"Prefer":"return=minimal"},
      body:JSON.stringify({source:"bol_mvp",started_at:started,finished_at:new Date().toISOString(),status:"success",records_written:records})
    });
    ...
    await db("sync_runs",{
      method:"POST",
      headers:{"Prefer":"return=minimal"},
      body:JSON.stringify({source:"bol_mvp",started_at:started,finished_at:new Date().toISOString(),status:"error",records_written:records,error_message:String(e)})
    }).catch(()=>{});
